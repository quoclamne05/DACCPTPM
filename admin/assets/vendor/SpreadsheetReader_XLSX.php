	...
	class SpreadsheetReader_XLSX implements Iterator, Countable
	{
		...
		public function next(): void
		{
			...
			if ($this -> RowOpen)
			{
				...
				while ($this -> Valid = $this -> Worksheet -> read())
				{
					switch ($this -> Worksheet -> name)
					{
						...
						case 'c':
							...
							$CurrentStyleId = (int)$this -> Worksheet -> getAttribute('s');

							// Get the index of the cell
							$CellIndex = $this -> Worksheet -> getAttribute('r');
							$Letter = preg_replace('{[^[:alpha:]]}S', '', $CellIndex);
							$CurrentCellIndex = self::IndexFromColumnLetter($Letter);
							...
							$this -> CurrentRow[$CurrentCellIndex] = '';

							$CellCount++;
							if ($CurrentCellIndex > $MaxIndex)
							{
						...
						case 'is':
							...
							if ($Value !== '' && $CurrentStyleId && isset($this -> Styles[$CurrentStyleId]))
							{
								$Value = $this -> FormatValue($Value, $CurrentStyleId);
							}
							elseif ($Value)
							{
							...
							$this -> CurrentRow[$CurrentCellIndex] = $Value;
							break;
					}
				}
				...
				{
					$EmptyRow = array_fill(0, $MaxIndex + 1, '');
					$MergedRow = array_replace($EmptyRow, array_filter($this -> CurrentRow, function($value) {
						return $value !== null;
					}));
					$this -> CurrentRow = $MergedRow;
